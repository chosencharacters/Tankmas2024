...
class MoleCircle extends BasicCircle
{
	...
	override public function new(p_x:Float = 120, p_y:Float = 160, graphicAssetPath:String = "assets/images/Circle Angry Faic.png",
			p_type:CircleType = CircleType.Mole, p_angleAmount:Circle_AngleAmount = Circle_AngleAmount.Two, p_force:Float = 50, p_min_first_angle:Float = 30,
			p_max_first_angle:Float = 85, p_min_second_angle:Float = 95, p_max_second_angle:Float = 150, p_startHp:Int = 1)
	{
		super(p_x, p_y, graphicAssetPath, p_type, p_angleAmount, p_force, p_min_first_angle, p_max_first_angle, p_min_second_angle, p_max_second_angle,
			p_startHp);
	...
	private function Launch()
	{
		...
		var circleAngle = Math.atan2(currentPosition.y - targetVector.y, targetVector.x - currentPosition.x) * FlxAngle.TO_DEG
			+ 180; // + 180 because unlike the OG Triple Triangle, the direction is down, and not up. Also flipped subtraction order.
